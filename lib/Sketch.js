...
function Sketch() {
  ...
  this.groupOrder = [];
  this.constraint = this.constraints = new IdList(Constraint, HConstraint);
  this.request = this.requests = new IdList(Request, HRequest);
  this.style = this.styles = new IdList(Style, HStyle);

  // These are generated from the above.
  this.entity = this.entities = new IdList(Entity, HEntity);
  this.param = this.params = new IdList(Param, HParam);
}
