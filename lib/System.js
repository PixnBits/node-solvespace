...
SolvespaceSystem.prototype.calculateRank = function calculateRank() {
  ...
  debug('calculateRank: mat.m', mat.m);
...
SolvespaceSystem.prototype.solveLinearSystem = function solveLinearSystem(arg0, arg1, arg2, arg3) {
  ...
  for (let i = 0; i < n; i += 1) {
    ...
    debug(`max ${max}, imax ${imax}`, Math.abs(max) < 1e-20);
    ...
    debug('A after row swapping', A);

    // swap(B[i], B[imax]);
    debug(`B[${i}] ${B[i]}, B[${imax}] ${B[imax]}`, B);
    const swapTemp = B[i];
    B[i] = B[imax];
    B[imax] = swapTemp;
    debug(`B[${i}] ${B[i]}, B[${imax}] ${B[imax]}`, B);
  ...
  debug('A should be in upper triangular form', A);
  // We've put the matrix in upper triangular form, so at this point we
  // can solve by back-substitution.
  for (let i = n - 1; i >= 0; i -= 1) {
    debug(`(|A[${i}][${i}]| = ${Math.abs(A[i][i])}) < 1e-20 ? ${Math.abs(A[i][i]) < 1e-20}`);
    ...
    let temp = B[i];
    debug(`temp = B[${i}] is ${temp}`, B);
    console.warn('should X be populated already?', X);
    for (let j = n - 1; j > i; j -= 1) {
      X[j] = X[j] || 0;
      debug(`temp -= X[j] * A[i][j]; ${temp} -= X[${j}] * A[${i}][${j}]; temp -= ${X[j]} * ${A[i][j]}; ${temp - (X[j] * A[i][j])};`, X);
      temp -= X[j] * A[i][j];
      // temp -= (X[j] || 0) * A[i][j];
    }
    debug(`setting X[${i}] = ${temp} / ${A[i][i]}`, A);
    X[i] = temp / A[i][i];
  }
  debug('A*X = B', A, X, B);
  debug('n', n);
...
SolvespaceSystem.prototype.solveLeastSquares = function solveLeastSquares() {
  ...
  for (let r = 0; r < mat.m; r += 1) {
    for (let c = 0; c < mat.m; c += 1) { // yes, AAt is square
      ...
      debug(`mat.AAt[${r}][${c}] = ${sum};`);
      mat.AAt[r] = mat.AAt[r] || [];
      mat.AAt[r][c] = sum;
    }
  }

  debug('mat.Z, mat.AAt, mat.B.num, mat.m', mat.Z, mat.AAt, mat.B.num, mat.m);
  ...
  for (let c = 0; c < mat.n; c += 1) {
    let sum = 0;
    for (let i = 0; i < mat.m; i += 1) {
      debug(`${sum} += mat.A.num[${i}][${c}] * mat.Z[${i}]; ${sum} += ${mat.A.num[i][c]} * ${mat.Z[i]}; ${sum + (mat.A.num[i][c] * mat.Z[i])}`, mat.Z);
      sum += mat.A.num[i][c] * mat.Z[i];
    }
    debug(`mat.X[${c}] = ${sum} * mat.scale[${c}]; ${sum * mat.scale[c]}`);
 ...
 * @returns Boolean
 */
SolvespaceSystem.prototype.writeJacobian = function writeJacobian(sketch, tag) {
  const system = this;
...
SolvespaceSystem.prototype.evalJacobian = function evalJacobian() {
  ...
  const { mat } = this;
  // debug('evalJacobian', mat.m, mat.n, mat.A.sym);
  for (let i = 0; i < mat.m; i += 1) {
    for (let j = 0; j < mat.n; j += 1) {
      // debug(`i: ${i}, j: ${j}, mat.A.sym: `, mat.A.sym);
      // debug(`mat.A.sym[${i}]:`, mat.A.sym[i]);
      // debug(`mat.A.sym[${i}][${j}]:`, mat.A.sym[i][j]);
      mat.A.num[i] = mat.A.num[i] || [];
 ...
 * @ param Group g
 *
 * SK (sketch) is no longer a global
 * @param Sketch sketch
 * @param hConstraint hc
 * @param Group g
 */
SolvespaceSystem.prototype.writeEquationsExceptFor = function writeEquationsExceptFor(sketch, hc, g) {
...
SolvespaceSystem.prototype.findWhichToRemoveToFixJacobian =
  function findWhichToRemoveToFixJacobian(sketch, g, bad, forceDofCheck) {
    for (let a = 0; a < 2; a += 1) {
      for (let i = 0; i < sketch.constraints.n; i += 1) {
        const c = sketch.constraints.elem[i];
        if (c.group.v !== g.h.v) {
          continue;
        }
        if (
        ...
        this.eq.clear();
        this.writeEquationsExceptFor(sketch, c.h, g);
        this.eq.clearTags();
...
SolvespaceSystem.prototype.newtonSolve = function newtonSolve() {
  ...
  for (let i = 0; i < mat.m; i += 1) {
    ...
    debug(`mat.B.num[${i}] = ${mat.B.num[i]}`, mat.B.sym[i]);
  }
  do {
...
SolvespaceSystem.prototype.solve = function solve(
  sketch, g, dof, bad, andFindBad, andFindFree, forceDofCheck
) {
  const system = this;
  var rankOk;
  var alone;
  ...
  function didntConverge() {
    ...
    debug('forEach vs for', system.eq.maximumId(), system.mat.n);
    system.eq.forEach((eq, eqKey) => {
      ...
      if (
        ...
        const c = sketch.constraint.findByIdNoOops(hc);
  ...
  // FIXME: replace string with hConstraint
  // this.writeEquationsExceptFor(sketch, 'no-constraint', g);
  this.writeEquationsExceptFor(sketch, Constraint.NO_CONSTRAINT, g);
  ...
  this.eq.forEach((e) => {
    ...
    e.tag = alone;
    p.tag = alone;
    this.writeJacobian(sketch, alone);
  ...
  if (!this.writeJacobian(sketch, 0)) {
  ...
  if (!rankOk) {
    if (!g.allowRedundant) {
      if (andFindBad) {
        this.findWhichToRemoveToFixJacobian(sketch, g, bad, forceDofCheck);
      }
    }
  } else {
  ...
  debug('solved, but where are the answers? Z or X maybe?', this.mat);
