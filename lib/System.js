...
// https://github.com/solvespace/solvespace/blob/master/src/system.cpp#L236-L277
SolvespaceSystem.prototype.solveLeastSquares = function solveLeastSquares(jacobian) {
  ...
  // write A*A'
  // (nth row of A) * (nth column of A')
  // (nth row of A) * (n' of row A')
  const AAt = [];
  for (let n = 0; n < A.num.length; n += 1) {
    AAt[n] = [];
    const aRow = A.num[n];
    for (let nPrime = 0; nPrime < A.num.length; nPrime += 1) {
      const aPrimeColumn = A.num[nPrime];
      if (aRow.length !== aPrimeColumn.length) {
        throw new Error('wut');
      }
      let sum = 0;
      for (let i = 0; i < aRow.length; i += 1) {
        const aEntry = aRow[i];
        const aPrimeEntry = aPrimeColumn[i];
        // debug(`solveLeastSquares: sum, ${sum} += ${aEntry} * ${aPrimeEntry}`);
        sum += aEntry * aPrimeEntry;
      }
      AAt[n][nPrime] = sum;
    }
  }
  ...
  if (!Z) {
    throw new Error('overconstrained? (no Z found)');
  }
